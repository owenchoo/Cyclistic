...
ALTER TABLE cyclistic..apr_tripdata

ALTER COLUMN start_station_id nvarchar(255);

-- Combining all of the data into one table

SELECT * INTO Q2_tripdata_

FROM cyclistic..apr_tripdata UNION ALL 
SELECT * FROM cyclistic..may_tripdata1 UNION ALL
SELECT * FROM cyclistic..may_tripdata2 UNION ALL
SELECT * FROM cyclistic..june_tripdata1 UNION ALL
SELECT * FROM cyclistic..june_tripdata2;

-- Checking the newly combined table

SELECT *

FROM cyclistic..Q2_tripdata_

-- the day_of_week datatype shouldn't be float, might cause some problems when analysing the data later on

ALTER TABLE cyclistic..Q2_tripdata_

ALTER COLUMN day_of_week nvarchar(255);

-- Changing the values to the name of days for easier readability

UPDATE cyclistic..Q2_tripdata_
