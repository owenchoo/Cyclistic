...
ALTER TABLE cyclistic..sep_tripdata2

ALTER COLUMN end_station_id nvarchar(255);

-- Combining all of the data into one table

SELECT * INTO Q3_tripdata

FROM cyclistic..aug_tripdata1 UNION ALL 
SELECT * FROM cyclistic..aug_tripdata2 UNION ALL
SELECT * FROM cyclistic..jul_tripdata1 UNION ALL
SELECT * FROM cyclistic..jul_tripdata2 UNION ALL
SELECT * FROM cyclistic..sep_tripdata1 UNION ALL
SELECT * FROM cyclistic..sep_tripdata2;

-- Checking the newly combined table

SELECT *

FROM cyclistic..Q3_tripdata

-- the day_of_week datatype shouldn't be float, might cause some problems when analysing the data later on

ALTER TABLE cyclistic..Q3_tripdata

ALTER COLUMN day_of_week nvarchar(255);

-- Changing the values to the name of days for easier readability

UPDATE cyclistic..Q3_tripdata
...
-- looking at the ride duration in descending order (casuals)
...
-- looking at the ride duration in descending order (members)
